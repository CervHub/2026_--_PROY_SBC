...
from lib.models.base import Field, FieldGroup, FullObservationField


class CervMantenimientoV07:
    observer_name: Field
    register: Field
    date: Field
    time: Field
    ...
    activity: FieldGroup
    observations: List[FullObservationField]
    comment: Field

    def __init__(self, observer_name: Field, register: Field, date: Field, time: Field, area: FieldGroup, department: FieldGroup, equipment: FieldGroup, activity: FieldGroup, observations: List[FullObservationField], comment: Field) -> None:
    ...
    @staticmethod
    def from_json(data: dict) -> "CervMantenimientoV07":
        ...
        observations = [FullObservationField.from_json(obs) for obs in data["observations"]]
        comment = Field.from_json(data["comment"])
        return CervMantenimientoV07(
            observer_name, register, date, time, area, department, equipment, activity, observations, comment
        )
